...
export function ExpenseList() {
  ...
  const { expenses, selectedCategory } = useAppSelector((state) => state.expenses)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <p className="text-lg font-semibold text-gray-900 dark:text-white">Total: ${totalAmount.toFixed(2)}</p>
      </div>
        ...
        {filteredExpenses.map((expense) => (
          ...
          >
            ...
            <div className="flex items-center gap-4">
              <span className="font-medium text-gray-900 dark:text-white">${Number(expense.amount).toFixed(2)}</span>
              <button
